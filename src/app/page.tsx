      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4">
        ...
        <section className="py-20">
          <h1 className="text-4xl font-bold">
            {personalConfig.name} {/* Displays the user's full name */}
            <span className="block text-2xl font-normal mt-2 text-neutral-600 dark:text-neutral-400">
              {contentConfig.home.title} {/* Displays the user's role/title */}
            </span>
          </h1>
          <p className="mt-6 text-lg max-w-2xl">
            {contentConfig.home.description} {/* Home page description */}
          </p>
        </section>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {contentConfig.home.projects.map((project) => (
              <div key={project.title} className="border rounded-lg p-6 bg-white/50 dark:bg-black/50 backdrop-blur-sm">
                <h3 className="font-semibold text-xl">{project.title}</h3> {/* Project title */}
